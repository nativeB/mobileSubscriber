...
export const subscriberPostRouteValidator = [
    ...
    check('msisdn', constants.ERROR_MESSAGES.msisdn.required).exists().matches(/^\+[1-9]\d{10,14}$/).withMessage(constants.ERROR_MESSAGES.msisdn.type)
  ]
export const subscriberPutRouteValidator = [
    ...
    check('msisdn').optional().exists().matches(/^\+[1-9]\d{10,14}$/).withMessage(constants.ERROR_MESSAGES.msisdn.type),
